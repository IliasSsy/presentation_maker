...
describe('resizeObject', () => {
    it('resizes text object', () => {
        const slide = addTextObject(createDefaultSlide('slide1'), createText('text1'))   
        expect(resizeObject(slide, 'text1', { width: 50, height: 90 }).elements[0]).toMatchObject({
            size: {width: 50, height: 90 }
    })   
    })   
    it('resizes figure', () => {
        const slide = { ...createDefaultSlide('slide1'), elements: [createTestRectangle()] }   
        expect(resizeObject(slide, 'rectangle1', { width: 100, height: 80 }).elements[0]).toMatchObject({size: { width: 100, height: 80 }})   
    })   
    it('does not mutate the original slide', () => {
